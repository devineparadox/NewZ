...
from AizenBot.utils.database import get_assistant, get_authuser_names, get_cmode
from AizenBot.utils.decorators import ActualAdminCB, AdminActual, language
from AizenBot.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Aizen.stop_stream_force(message.chat.id)
    except:
        pass

    userbot = await get_assistant(message.chat.id)
    try:
        chat_id = message.chat.username or message.chat.id
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Aizen.stop_stream_force(chat_id)
        except:
            pass
